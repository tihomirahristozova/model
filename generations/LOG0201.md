...
Unlike "[LOG0202 Create Unmanaged Store transaction](https://confluence.erp.net/display/techdoc/LOG0202+Create+Unmanaged+Store+transaction)" the current generation checks whether "Direct order execution" in the Store’s definition is check-marked and creates a sub-document if it is. Otherwise, no sub-document is created.
...
Note: The new store transaction is saved then and only when there is at least one line in it.
