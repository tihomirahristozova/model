...
This Generation is used to create Store Orders for all products from the current Sale Order that **are stocked** (Product.ProductType.IsStocked = True). Unlike [CRM0104](https://olddocs.erp.net/tech/crm0104-create-issue-store-orders-for-products-that-aren-t-shipped-572489731.html) this generation procedure does not check whether the products **is shipped or not** (Product.ProductType.IsShipped = True/False). 
